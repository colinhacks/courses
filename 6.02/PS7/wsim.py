...
#		print 'attaching ', 1.0*load/numnodes, ' to node ', n.get_id()
    ...
    def fairness(self, rate_normalized):
        ...
        for n in self.nlist:
            ...
            succ_sumsq += x*x
        if succ_sumsq > 0:
            return (succ_sum**2)/((nn-1.0) * succ_sumsq) - 1.0/(nn-1.0)
        else:
            return 0
...
class Network(WirelessNetwork):
    def __init__(self,tsim,nnodes,psize,skew):
        WirelessNetwork.__init__(self,nnodes,'*',psize,'exponential',
                                 100,True,'Mine',skew,0,tsim+1)
    def sim0(self, step=100):
        nn = self.config.numnodes
        nstep = self.config.simtime/step
        th = numpy.zeros((nstep,nn))
        for s in xrange(nstep):
            self.step(step)
            for n in xrange(nn):
                th[s,n] = self.nlist[n].stats.success
        steps = numpy.linspace(step,nstep*step,nstep)
        th = th * self.config.ptime
        u = th.sum(1)/steps
        ths = th.sum(1)
        ths2 = (th**2).sum(1)
        ths[ths2<=0.0] = 1.0
        ths2[ths2<=0.0] = 1.0
        f = (ths**2)/(ths2*(nn-1.0))-1/(nn-1.0)
        return (th,u,f,steps)
    def sim(self, step=100):
        (th,u,f,steps) = self.sim0(step)
        print 'utilization:', u[-1]
        print 'fairness:   ', f[-1]
        p.figure(1)
        p.subplot(211)
        p.plot(steps,th,'.-')
        p.grid()
        p.subplot(212)
        p.plot(steps,u,'r.-',steps,f,'b.-')
        p.grid()
        p.show()

class Node(WirelessNode):
    def __init__(self,location,network,retry):
