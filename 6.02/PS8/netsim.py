...
class Router(Node):
    ...
    ADVERT_INTERVAL = 10  # time between route advertisements
    ...
    def getlink(self, nbhr):
        if self.address == nbhr: return 'Self'
        for l in self.links: 
    ...
    def process_advertisement(self,p,link,time):
        self.integrate_advertisement(p.properties['ad'],link,time)
...
class RouterNetwork(Network):
    def __init__(self,SIMTIME,NODES,LINKS,LOSSPROB=0.0):
...
# correct: {nodename: (allcosts,routeselsewhere)}, e.g.,
# {'A': ({'A':0,'B':1.1,}, {'B':('A','C',),})}
numtypes = (type(0),type(0.0))
def rtverify(net,correct,verbose=True,tol=0.01):
    for source in correct:
        c0 = correct[source][0]     # correct costs at the source
        src = net.addresses[source]
        c = src.spcost              # actual costs at the source
        for rid in c0:
            if rid not in c:
                if verbose:
                    print 'missing distance entry for %s at %s'%(rid,source)
                return False
            if type(c[rid]) not in numtypes:
                if verbose:
                    print 'non-numeric distance entry for %s at %s'%(rid,source)
                return False
            if not abs(c0[rid]-c[rid])<tol:
                if verbose:
                    print 'incorrect distance entry for %s at %s:'%(rid,source)
                    print '     %f (should be %f)'%(c[rid],c0[rid])
                return False
        for rid in c:
            if rid not in c0:
                if verbose:
                    print 'extra distance entry for %s at %s'%(rid,source)
                    print '   cost: %f'%c[rid]
                return False
        r0 = correct[source][1]     # correct routing names at the source
        r = src.routes              # actual routing table
        for rid in r0:
            if rid not in r:
                if verbose:
                    print 'missing router entry for %s at %s'%(rid,source)
                return False
            if not isinstance(r[rid],Link):
                if verbose:
                    print 'router entry not a Link for %s at %s'%(rid,source)
                return False
            route = r[rid]
            node = route.end2 if src==route.end1 else route.end1
            dest = node.address
            if dest not in r0[rid]:
                if verbose:
                    print 'incorrect route for %s at %s:'%(rid,source)
                    print ' %s not in [%s]'%(dest,','.join(r0[rid]))
                return False
        for rid in r:
            if (rid not in r0) and (rid != source):
                if verbose:
                    print 'extra router entry for %s at %s'%(rid,source)
                    print '   cost: %f'%c[rid]
                return False
    return True
