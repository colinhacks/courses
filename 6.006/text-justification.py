...
case4 = "Call me Ishmael. Some years ago - never mind how long precisely - having little or no money in my purse, and nothing particular to interest me on shore, I thought I would sail about a little and see the watery part of the world. It is a way I have of driving off the spleen, and regulating the circulation. Whenever I find myself growing grim about the mouth; whenever it is a damp, drizzly November in my soul; whenever I find myself involuntarily pausing before coffin warehouses, and bringing up the rear of every funeral I meet; and especially whenever my hypos get such an upper hand of me, that it requires a strong moral principle to prevent me from deliberately stepping into the street, and methodically knocking people's hats off - then, I account it high time to get to sea as soon as I can."
def layout(x):
  print x
  print "\n\n###############\n### BEGIN ####\n#############"
  def value(L,i,j):
    if True:#i==0: special
      print "\n--------------\nFor i=",i," and j=",j
    if j<len(L) and i+j<len(L):
      good=10.* (L[i+j][-1] in punct)
    else:
      good=0
    print "Good: ",good
    if j-1==0 or minwidth(L[i:i+j+1]) > 468:
      bad=float('inf')
    else:
      print L[i:i+j+1]
      bad=((468-minwidth(L[i:i+j+1]))/(j-1))**2
    print "BAD: ",bad
    if True:#i==0: #special
      print "Badness for L words ",i, " thru ",j," is: ",bad-good
      print "with additional: ",DP[i+j+1][0]
    print "TOTAL:",bad-good+DP[i+j+1][0]
    return bad-good

  L=x.split()

  DP={len(L):(0,None),len(L)-1:(float('inf'),None)}
  doc = PDF()
  punct={".",",",";",":"}
  n=len(L)

  print "DP: ",DP
  for i in reversed(range(n-1)):#reversed([n-5,n-4,n-3,n-2,n-1]):  # Decreasing from n-1 to 1
    print "###########\nValue: ",i
    vect=[(value(L,i,j)+DP[i+j+1][0],j) for j in range(1,n-i) ]#n-i+1
    print "vector: \n",vect
    val=min(vect,key=lambda x:x[0])
    DP[i] = val
    print "\n-------------\n \"",L[i],"\": ",val
  print DP
  print "L 1-13"
  ...
  lines=[]
  while t<len(L):
    print "LINE from ,",t," to ", t+DP[t][1],":\n"

    print L[t:t+DP[t][1]+1]
    if DP[t][1]==None:
      a=len(L)
    lines.append(L[t:t+DP[t][1]+1])
    t=t+DP[t][1]+1
  
  for num in range(len(lines)):
    words=lines[num]
    space = (468-sum([width(w) for w in words]))/(len(words)-1)
    cumulative=0
    for index in range(len(words)):
      #print "Cumul: ",cumulative
      doc.write(cumulative, 14*(num+1), words[index])
      if words[index]=="misadventured":#special
        print "x: ",cumulative
        print "y: ", 14*(num+1)
      cumulative+=width(words[index])+space
