...
#for a in boost_1796.classifiers:
    #print a
...
def most_misclassified(classifier, n=5):
    ...
    """
    print classifier.data[0:2]
    a=sorted(zip(classifier.data,classifier.data_weights),key=lambda x:-1*x[1])
    return [legislator_info(b[0]) for b in a[:5]]
...
WHAT_I_FOUND_INTERESTING = "Getting how the structure of a neural net changes it response to training data."
WHAT_I_FOUND_BORING = "Changing a bunch of arbitrary parameters for hours trying to get a 2-2-1 neural net to classify everything correctly.  A 4-1 net is really hacky and shouldn't have worked."
