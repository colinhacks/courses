...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        returns: number (float or int)
        """
        #print [type(a) for a in self.get_weights()]
        value = sum([a.output()*b.get_value() for (a,b) in zip(self.get_inputs(),self.get_weights())])
        #print "value: ",value
        try:
            a = sigmoid(value)
    ...
    def compute_doutdx(self, elem):
        ...
        if elem in self.my_weights:
            ...
            for weight in self.my_weights:
                if True:#self.isa_descendant_weight_of(elem,weight):
                    index = self.my_weights.index(weight)
                    answer += weight.get_value()*self.my_inputs[index].dOutdX(elem)
            return self.output()*(1-self.output())*answer
...
def make_neural_net_two_layer():
    ...
    r=[5.597782, -8.508484, -10.009850, -1.063481, 3.006750, 1.889923, 1.631559, -4.702782, -5.102646, -7.175330, 6.983782, -2.782395, -9.010955, -1.723730, -3.722417]
...
def make_neural_net_challenging():
    ...
    i1 = Input('i1', 0.0)
    i2 = Input('i2', 0.0)
    #a={'w1A': 7.615146,'w2A': -3.887937,'wA': 11.867216,'wAD': -3.567757,'wBD': -0.418131,'wD': -2.450926,'w1B': 2.333972,'w2B': -6.271935,'wB': -3.748149,'wAC': -14.167363,'wBC': 4.631434,'wC': -1.888854,'wAE': -9.136848,'wBE': -3.431937,'wCE': -13.856196,'wDE': 4.153478,'wE': -3.723325}


    w1A = Weight('w1A', random_weight()) #['w1A']+random.random()-.5)#,random_weight())  
    w2A = Weight('w2A', random_weight()) #['w2A']+random.random()-.5)#,random_weight()) random_weight()) 
    w1B = Weight('w1B', random_weight()) #['w1B']+random.random()-.5)#,random_weight())
    w2B = Weight('w2B', random_weight()) #['w2B']+random.random()-.5)#,random_weight())
    w1C = Weight('w1C', random_weight()) #['w1A']+random.random()-.5)#,random_weight())  
    w2C = Weight('w2C', random_weight()) #['w2A']+random.random()-.5)#,random_weight()) random_weight()) 
    w1D = Weight('w1D', random_weight()) #['w1B']+random.random()-.5)#,random_weight())
    w2D = Weight('w2D', random_weight())


    wAE = Weight('wAE', random_weight()) #['wAE']+random.random()-.5)#,random_weight())
    wBE = Weight('wBE', random_weight()) #['wBE']+random.random()-.5)#,random_weight())
    wCE = Weight('wCE', random_weight()) #['wCE']+random.random()-.5)#,random_weight())
    wDE = Weight('wDE', random_weight()) #['wDE']+random.random()-.5)#,random_weight())
...
def make_neural_net_with_weights():
    ...
    init_weights = {'w1A': -4.092520,'w2A': 2.797151,'wA': 2.129699,'w1B': -1.215812,'w2B': -1.157917,'wB': -0.639209,'w1C': -2.311259,'w2C': -2.270531,'wC': -3.957341,'w1D': 2.964050,'w2D': -4.259387,'wD': 2.380623,'wAE': -8.989630,'wBE': 2.237896,'wCE': 6.237631,'wDE': -8.961532,'wE': -4.155782}
