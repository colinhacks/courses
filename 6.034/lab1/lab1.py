...
# family_rules.
family_rules = [    IF( OR( 'male (?x)', # All people given same-identifty rule
                            'female (?x)'),
                    THEN('same-identity (?x) (?x)')),

                    IF( AND('brother (?x) (?y)',# sibling reflexivity
                            'female (?y)'),
                    THEN( 'sister (?y) (?x)' )),
                    ...
                    IF( AND('sister (?x) (?y)',# sibling reflexivity
                            'male (?y)'),
                    THEN( 'brother (?y) (?x)' )),
                    ...
                    IF( AND('parent (?x) (?y)',#father, son
                            'male (?x)',
                            'male (?y)' ),
                    THEN( 'father (?x) (?y)',
                          'son (?y) (?x)')),

                    IF( AND('parent (?x) (?y)',# father/daughter
                            'male (?x)',
                            'female (?y)' ),
                    THEN( 'father (?x) (?y)',
                          'daughter (?y) (?x)')),

                    IF( AND('parent (?x) (?y)',# mother/son
                            'female (?x)',
                            'male (?y)' ),
                    THEN( 'mother (?x) (?y)',
                          'son (?y) (?x)')),

                    IF( AND('parent (?x) (?y)',# mother/daughter
                            'female (?x)',
                            'female (?y)' ),
                    THEN( 'mother (?x) (?y)',
                          'daughter (?y) (?x)')),
                    ...
                    IF( AND('male (?x)',# Brother based on shared parents
                            'parent (?y) (?x)',
                            'parent (?y) (?z)',
                    ...
                    IF( AND('female (?x)',# Sister based on shared parent
                            'parent (?y) (?x)',
                            'parent (?y) (?z)',
                            NOT( 'same-identity (?x) (?z)' ),
                            ),
                    ...
                    IF( AND('parent (?x) (?y)',# Cousins
                            'parent (?z) (?w)',
                            OR('sister (?x) (?z)',
                               'brother (?x) (?z)'),
                            NOT('same-identity (?y) (?w)')),
                    THEN( 'cousin (?x) (?y)',
                          'cousin (?y) (?x)')),
...
WHAT_I_FOUND_INTERESTING = 'The complexity of what is generated from just a few rules.  Also the Harry Potter trivia. Great example.'
